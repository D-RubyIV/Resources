...
if is_windows:
    os.system("title Dinne Offucates")
...
def rename(filename: str):
    ...
    for func in funcs:
        if func.name == "__init__":
            continue
        newname = "".join(random.choice(['I','J']) for i in range(random.randint(2, 20)))
        while newname in used:
            newname = "".join(random.choice(['I','J']) for i in range(random.randint(2, 20)))
        while True:
            if newname in allName:
                newname = "".join(random.choice(['I','J']) for i in range(random.randint(2, 20)))
                print('===================================================')
            else:
                break
        
        allName.append(newname)

        used.add(newname)
        pairs[func.name] = newname
    print('Rename Funcition.......')
    ...
    for i in range(len(originals)):
        for key in newpairs:
            originals[i] = re.sub(r"({.*)(" + key + r")(.*})", "\\1" + newpairs[key] + "\\3", originals[i], re.MULTILINE)

    while True:
        found = False
        code = do_rename(newpairs, code)
        for key in newpairs:
...
with open(f"{file[:-3]}-obf.py", "w", encoding='utf-8') as f:
    f.write(code)
