...
class ADB:
    ...
    def GetPosXml(self, emulator, element):
        pos = []
        try:
            tree = html.parse(self.DumXml(emulator))
...
class LDPlayer:
    ...
    def ExecuteLD(self, shell):
        return str(subprocess.Popen(f"ldconsole {shell}", stdout=subprocess.PIPE, shell=True, cwd=self.pathLD).stdout.read())
    ...
    def SendText(self, text=None, VN=None):
        if text == None:
            text =  str(base64.b64encode(VN.encode('utf-8')))[1:]
    ...
    def Swipe(self, x1, y1, x2, y2):
        self.AdbLd(f"shell input touchscreen swipe {x1} {y1} {x2} {y2}")
    ...
    def Create(self, Name):
        self.ExecuteLD(f"add --name {Name}")
    ...
    def InstallAppFile(self, File_Name):
        self.ExecuteLD(f"installapp --{self.param} {self.NameOrId} --filename '{File_Name}'")
